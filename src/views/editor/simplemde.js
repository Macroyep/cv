...
function createIcon(options, enableTooltips, shortcuts) {
  ...
  if (options.title && enableTooltips) {
    ...
    if (options.text) {
      el.innerHTML = `<span style="padding-left:3px;">${options.text}</span>`;
    }
  }
...
function drawCustomBlock(editor, block) {
  var cm = editor.codemirror;
  _replaceSelection(cm, undefined, ['', block]);
}
...
SimpleMDE.prototype.drawCustomBlock = function(block) {
  drawCustomBlock(this, block);
};
